# Modular Word2Vec
## About the project
Modular approach to the definition and training problems, using PyTorch-like
declarative approach with extensibility for future layer types, activation
...
- <input type="checkbox" disabled checked> Implement SGD Optimizer:
    - <input type="checkbox" disabled checked> Optimizer: Make graph on first model pass
        - <input type="checkbox" disabled checked> Model and Layer: register in graph
    - <input type="checkbox" disabled checked> Add loss and disable graph making
    - <input type="checkbox" disabled checked> Solve graph backwards, merging known combinations
...
- <input type="checkbox" disabled checked> Implement data preparation via Jupyter notebook
...
## References
- text8 dataset
- micrograd by Andrew Carpathy
- Efficient Estimation of Word Representations in Vector Space (CBOW paper)
